...
FILE_CDMX_CELLS <- "/cells_cdmx.geojson"
FILE_CSMX_CENTROIDS <- "/iso_centroids_cdmx.geojson"
...
We use a python code with Selenium to get the data of all charger direction from the official [Tesla site](https://www.tesla.com/es_mx/findus/list/chargers/Mexico)
...
nowwe get the official geographic information from Mexico INEGI's National Road Network (Red Nacional de Caminos, from spanish). 

``` bash

# Download zip ( ~660 Mb)
wget "https://www.inegi.org.mx/contenidos/productos/prod_serv/contenidos/espanol/bvinegi/productos/geografia/caminos/2024/794551132166_s.zip" -O ./data/raw/rnc_2024.zip

# Unzip files
unzip -o ./data/raw/rnc_2024.zip -d ./data/raw/nat_road_net_mex

# Remove ZIP file 
rm ./data/raw/rnc_2024.zip
...
agg_cell <- censo_sf |>
  group_by(CELL) |> 
  summarise(
    POBTOT = sum(Pdata/experiments/cdmx_bjOBTOT, na.rm = TRUE),
    .groups = "drop"
  ) 
...
results_nation <- readr::read_csv(paste0(SAMPLE_DATA, "/cdmx_final_points.csv")) 
knitr::kable(results_nation)
...
cdmx_poly <- st_read(paste0(SAMPLE_DATA,SHP_CDMX), quiet = TRUE)
cells_cdmx <- st_read(paste0(SAMPLE_DATA,FILE_CDMX_CELLS), quiet = TRUE)
opt_points_sf <- st_read(paste0(SAMPLE_DATA,FILE_CSMX_CENTROIDS), quiet = TRUE)
...
![Population coverage (%) by municipallity.](./images/dens_mx_coverages.png)

Both Azcapotzalco and Benito Juárez have populations that are fully within our 90-minute reach. 