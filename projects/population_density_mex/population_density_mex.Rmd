...
opt_points_sf |> 
  ...
  st_drop_geometry() |>
  select(lng, lat) |> write_csv(paste0(SAMPLE_DATA, "/cdmx_final_points.csv"))
...
car_iso15 <- st_read(paste0(CLEAN_DATA,FINAL_ISO_CDMX_C), quiet = TRUE)
mb_iso15 <- st_read(paste0(SAMPLE_DATA,FINAL_ISO_CDMX_M), quiet = TRUE)
...
pal <- colorNumeric(
  palette = c("#f2fff5","#c7f7d6","#34c277", "#007a3d"   
), 
...
m_pop <- leaflet( options = leafletOptions(minZoom = 9, zoomControl = FALSE)) |>
...
Lets see the general coverage for each municipality in terms of isochrones. 
...
We calculate some statistics about the area coverages in terms og geographical coverage:
...
iso15_mp <-
  st_intersection(
    iso15 |> st_make_valid() |> mutate(id1 = row_number()) |> select(id1),
    iso15 |> st_make_valid() |> mutate(id2 = row_number()) |> select(id2)
  ) |>
  # keep only *true* pairwise intersections (no self, no duplicates)
  filter(id1 < id2, !st_is_empty(geometry)) |>
  st_union() |>
...
area_m2 <- st_area(geom_clean)  # vector of areas (m²)
total_area_m2 <- as.numeric(st_area(st_union(geom_clean)))  # single total area


area_cdmx <- st_area(poly_cdmx)        
area_isos <- st_area(st_union(iso15 |> st_make_valid() ))   
area_inter <- st_area(iso15_mp |> st_make_valid())  
pop_area <- st_area(st_union(cdmx_cells_sf |> st_make_valid() ))

poly_cdmx_v <- poly_cdmx |>
  st_make_valid()

iso15_v <- iso15 |>
  st_make_valid() |>
  st_transform(st_crs(poly_cdmx_v))   # only if needed

# Union all iso15 polygons into one mask
iso15_union <- st_union(iso15_v)

# Intersection between CDMX and union of iso15
inter_cdmx_iso15 <- st_intersection(poly_cdmx_v, iso15_union)

# Percentage of CDMX covered
pct_cdmx_covered <-
  100 * as.numeric(st_area(inter_cdmx_iso15)) /
        as.numeric(st_area(poly_cdmx_v))
